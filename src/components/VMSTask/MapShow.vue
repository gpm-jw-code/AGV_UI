<template>
  <div class="border w-100 py-2 px-2 d-flex flex-row bg-light">
...
export default {
  ...
  async mounted() {
    ...
    setTimeout(() => {
      MapAPI.GetMapFromLocal().then((map) => {
        this.loading = false;
        if (map == undefined) {
          Notifier.Danger('圖資取得失敗(後端伺服器異常)', 'bottom', 5000);
        }
        else if (map.Points == undefined) {
          Notifier.Danger('圖資取得失敗(AGVS伺服器異常)', 'bottom', 5000);
        }
        else {
          this.map_data = map;
  ...
  methods: {
    ...
    AGVStyleRender() {
      ...
      //   this.map.getLayers().item(2).getSource().getFeatures().forEach((feature) => {
